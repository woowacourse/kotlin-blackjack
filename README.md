...
### 2단계 추가된 요구사항
- 돈
    - [X] 베팅 금액은 최소 1원 이상 최대 3억원(300,000,000)까지 가능하다.
    - [X] 베팅 금액은 베팅 금액을 가진다.
- 블랙잭
    - [X] 카드가 두 장이면서, 카드의 합이 21일 경우 블랙잭이 된다.
    - [X] 플레이어가 블랙잭일 경우 베팅 금액의 1.5배를 받는다.
        - 단, 딜러도 블랙잭일 경우 베팅 금액을 다시 돌려받는다.
- 결과
    - [X] 게임이 끝난 후, 각 참가자(딜러/플레이어)의 수익을 출력한다.
- 플레이어
    - [X] 플레이어는 최소 1명 이상 8명까지만 게임에 참여할 수 있다.
