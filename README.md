...
## 🛠️ (2단계) 구현할 기능
- [x] 플레이어는 각자의 배팅액을 가지고 있다.
  - [x] 배팅액은 음수일 수 없다.
- [x] 플레이어는 각자의 자금을 가지고 있다.
  - [x] 모든 플레이어는 배팅한 금액만큼 자금은 마이너스로 시작한다.
- [x] 처음 두 장의 카드 합이 21일 경우 블랙잭이 되면 베팅 금액의 1.5 배를 딜러에게 받는다.
- [x] 딜러와 플레이어가 모두 동시에 블랙잭인 경우 플레이어는 베팅한 금액을 돌려받는다.
- [x] 플레이어가 패배한 경우, 플레이어는 배팅한 금액을 잃는다.
- [x] 플레이어가 승리한 경우, 플레이어는 배팅한 금액의 1배를 딜러에게 받는다. 
## 🖥️ 출력 예시
...
딜러 카드: 9다이아몬드, 4하트, Q클로버 - 결과: 버스트
시아 카드: J스페이드, 5하트 - 결과: 15
