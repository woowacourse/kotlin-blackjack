...
- 게임을 완료한 후 각 플레이어별로 수익를 출력한다.

## 블랙잭 게임 승리 규칙
- 블랙잭은 최초 2장의 카드가 21일 경우다
- 딜러와 플레이어 모두 블랙잭일 경우 무승부이며 베팅금액을 돌려받는다
- 플레이어는 블랙잭으로 승리할 경우 베팅 금액의 1.5배를 받는다.
- 플레이어는 블랙잭이 아닐 때 승리할 경우 베팅 금액의 1.0배를 받는다.
- 딜러와 플레이어가 모두 버스트될 경우 플레이어는 베팅 금액을 잃는다.
  - 딜러는 플레이어들의 베팅 금액을 받는다.
- 플레이어가 패배할 경우 베팅 금액을 모두 잃는다
- 딜러는 승리할 경우 패배한 플레이어의 베팅 금액을 받는다.
