...
class OutputView {
    ...
    fun printGetCardMore() {
        println(MESSAGE_INPUT_YES_OR_NO)
    }
    ...
    fun printPlayerStatus(participant: Participant) {
        ...
        print(MESSAGE_PARTICIPANT_STATUS.format(participant.cards.toString()))
    }

    fun printAllPlayerStatusResult(participants: List<Participant>) {
        for (participant in participants) {
            print(MESSAGE_PARTICIPANT_STATUS.format(participant.name.value, participant.cards.cards.toString()))
    ...
    fun printFinalResult(gameResult: GameResult) {
        ...
        println(MESSAGE_DEALER_RESULT.format(gameResult.getDealerWinResult(), gameResult.getDealerWinResult()))
    ...
    companion object {
        ...
        private const val MESSAGE_INPUT_YES_OR_NO = "는/은 한장의 카드를 더 받겠습니까?(예는 y, 아니오는 n)"
