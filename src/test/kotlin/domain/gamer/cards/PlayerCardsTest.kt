...
class PlayerCardsTest {
    ...
    fun `뽑은 카드를 저장한다`(cards: List<Card>) {
        ...
        assertThat(playerState.cards).isEqualTo(cards)
    }
    ...
    fun `카드 값의 합을 반환한다`() {
        val playerState =
            PlayerCards(mutableListOf(Card(Shape.SPADE, CardValue.JACK), Card(Shape.SPADE, CardValue.JACK)))
    ...
    fun `플레이어 카드의 합이 21이 넘었을 경우 true를 반환한다`() {
        ...
        assertThat(playerState.checkOverCondition()).isTrue
    }
    ...
    fun `플레이어 카드의 합이 21이 넘지 않았을 경우 false를 반환한다`() {
        val playerState =
            PlayerCards(mutableListOf(Card(Shape.SPADE, CardValue.JACK), Card(Shape.SPADE, CardValue.JACK)))
        assertThat(playerState.checkOverCondition()).isFalse
    }
}
