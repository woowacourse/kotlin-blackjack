...
class PlayerTest {
    ...
    fun `에이스 카드를 가지고 버스트 되지 않았으면 카드 총합에 10을 더한다`() {
        player.addCard(TrumpCard(Denomination.ACE, Suit.DIA))
        player.addCard(TrumpCard(Denomination.NINE, Suit.HEART))
    ...
    fun `에이스 카드를 가지고 버스트 되었으면 카드 총합을 유지한다`() {
        player.addCard(TrumpCard(Denomination.ACE, Suit.DIA))
        player.addCard(TrumpCard(Denomination.SEVEN, Suit.HEART))
        player.addCard(TrumpCard(Denomination.NINE, Suit.HEART))
    ...
    fun `최초에 카드를 받은 후 오픈할 카드 2장을 반환한다`() {
        ...
        assertThat(player.getInitialCards())
            .containsExactly(TrumpCard(Denomination.SEVEN, Suit.HEART), TrumpCard(Denomination.ACE, Suit.DIA))
    }
