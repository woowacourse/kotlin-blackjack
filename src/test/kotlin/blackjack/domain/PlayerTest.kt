...
class PlayerTest {
    ...
    fun `플레이어는 모든 카드의 합이 21 미만이 될 수 있을 경우 계속해서 카드를 뽑을 수 있다`() {
        val player = Player(name = "Eden")
        val card1 = Card(Number.SEVEN, Suit.HEART)
        val card2 = Card(Number.SEVEN, Suit.DIAMOND)
        val card3 = Card(Number.SEVEN, Suit.DIAMOND)
        player.getCards(listOf(card1, card2, card3))
        assertThrows<IllegalArgumentException> { player.getCard(Card(Number.TWO, Suit.SPADE)) }
    }
