...
class BlackjackTest {
    ...
    @Test
    fun `블랙잭 상태일 때 베팅하면 에러가 발생한다`() {
        val state = Blackjack(Hand(listOf()), null)
    ...
    @Test
    fun `블랙잭 상태이고 베팅을 했을 때 수익을 달라고 하면 베팅 금액과 1점5를 곱한 값을 반환한다`() {
        val betAmount = 10000
        val state = Blackjack(Hand(listOf()), Money(betAmount))

        val actual = state.getProfit()

        assertThat(actual).isEqualTo(betAmount * 1.5)
    }

    @Test
    fun `블랙잭 상태이고 베팅을 하지 않았을 때 수익을 달라고 하면 에러가 발생한다`() {
        val state = Blackjack(Hand(listOf()), null)
