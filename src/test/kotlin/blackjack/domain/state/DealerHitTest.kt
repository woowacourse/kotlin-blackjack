...
class DealerHitTest {
    ...
    fun `카드를 뽑았을때 21이 넘는다면 Burst객체를 반환한다`() {
        val hit = DealerHit(CardBunchForState(Cards.nine, Cards.eight))
        assertThat(hit.draw(Cards.jack)).isInstanceOf(Burst::class.java)
    }

    @Test
    fun `카드를 뽑았을때 16이 넘지 않는다면 DealerHit객체를 반환한다`() {
        val hit = DealerHit(CardBunchForState(Cards.two, Cards.eight))
    ...
    fun `카드를 뽑았을때 21이 넘는다면 Burst객체를 반환한다(카드 2번 뽑음)`() {
        val hit = DealerHit(CardBunchForState(Cards.two, Cards.three))
