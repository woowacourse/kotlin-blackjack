...
class StayTest {
    ...
    @Test
    fun `스테이 상태일 때 베팅하면 에러가 발생한다`() {
        val state = Stay(Hand(listOf()), null)
    ...
    @Test
    fun `스테이 상태이고 베팅을 했을 때 수익을 달라고 하면 베팅 금액과 스테이 상태의 수익률을 곱한 값을 반환한다`() {
    ...
    @Test
    fun `스테이 상태이고 베팅을 하지 않았을 때 수익을 달라고 하면 에러가 발생한다`() {
        val state = Stay(Hand(listOf()), null)
