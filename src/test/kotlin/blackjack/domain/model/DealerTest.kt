...
class DealerTest {
    ...
    fun `플레이어별 최종 수익을 반환한다 1`() {
        ...
        val playersProfits: Map<Player, Int> = dealer.getPlayersProfits(listOf(player1, player2))
    ...
    fun `플레이어별 최종 수익을 반환한다 2`() {
        ...
        val playersProfits: Map<Player, Int> = dealer.getPlayersProfits(listOf(player1, player2, player3, player4))
        val actual: Map<Player, Int> =
            mapOf(
                player1 to -11111,
                player2 to 0,
    ...
    fun `딜러의 최종 수익을 반환한다 1`() {
        ...
        val player2 = Player("jason", 20000, Card(Suit.CLUB, Rank.SEVEN), Card(Suit.SPADE, Rank.KING)) // 17점
        val playersProfits: Map<Player, Int> = dealer.getPlayersProfits(listOf(player1, player2))
        val dealerProfit: Int = dealer.getDealerProfit(playersProfits)
