...
class HitTest {
    ...
    @Test
    fun `Hit 상태에서 card가 2장이면 시작이다`() {
        assertThat(hit.isStarted()).isFalse()
    }
    ...
    fun `딜러 전략일때 카드가 17이상일 경우 스테이를 반환한다`() {
        ...
        hit.nextState(HEART_TWO)
        assertThat(hit.nextState(HEART_TWO)).isInstanceOf(Stay::class.java)
    }

    @Test
    fun `플레이어 전략일때 카드가 21일 경우 스테이를 반환한다`() {
        hit.nextState(HEART_SIX)
        assertThat(hit.nextState(HEART_FOUR)).isInstanceOf(Stay::class.java)
    }

    @Test
    fun `카드를 받을때 16이하 일 경우 Hit를 반환한다`() {
        assertThat(hit.nextState(HEART_FOUR)).isInstanceOf(Hit::class.java)
    }
