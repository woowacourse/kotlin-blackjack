...
class CardGameTest {
    ...
    fun `카드를 플레이어별로 2장씩 랜덤으로 지급한다`() {
        ...
        assertThat(player1.hand.size()).isEqualTo(2)
        assertThat(player2.hand.size()).isEqualTo(2)
    }

    @Test
    fun `플레이어 두 명의 정보를 생성한다`() {
        val players = game.initPlayers(Names(listOf(Name("jason"), Name("pobi")))).toList()
        assertThat(players.size).isEqualTo(2)
        assertThat(players[0].name.value).isEqualTo("jason")
        assertThat(players[0].hand.size()).isEqualTo(2)
        assertThat(players[1].name.value).isEqualTo("pobi")
        assertThat(players[1].hand.size()).isEqualTo(2)
    }
