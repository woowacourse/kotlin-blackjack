...
class DealerTest {
    ...
    @Test
    fun `에이스가 있을 때 에이스가 11로 계산 되어도 버스트 되지 않고 총합이 16을 초과하면 카드를 더 이상 뽑지 않는다`() {
        val dealer = Dealer(ParticipantCards())
    ...
    @Test
    fun `에이스가 있을 때 에이스가 11로 계산 되어도 버스트 되지 않고 총합이 16을 초과하지 않으면 카드를 한 장 더 뽑는다`() {
        val dealer = Dealer(ParticipantCards())
    ...
    @Test
    fun `에이스가 있을 때 에이스가 11로 계산 되면 버스트 되고 에이스를 1로 계산했을 때 총합이 16을 초과하면 카드를 더 이상 뽑지 않는다`() {
        val dealer = Dealer(ParticipantCards())
    ...
    @Test
    fun `에이스가 두 개일 때 버스트 되지 않고 16을 초과하지 않으면 카드를 한 장 더 뽑는다`() {
        val dealer = Dealer(ParticipantCards())
    ...
    @Test
    fun `에이스가 여러 개일 때 버스트 되지 않고 16을 초과하면 카드를 더 이상 뽑지 않는다`() {
        val dealer = Dealer(ParticipantCards())
