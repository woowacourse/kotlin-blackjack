...
class GameJudgeTest {
    ...
    fun `카드 2장의 합이 21이면 블랙잭 상태를 반환한다 `() {
        ...
        val hand = Hand().make(HEART_ACE, HEART_QUEEN)
        val actual = GameJudge.judge(hand)
    ...
    fun `카드 3장의 합이 21이면 스테이 상태를 반환한다 `() {
        ...
        val hand = Hand().make(HEART_TWO, HEART_QUEEN, CLOVER_NINE)
        val actual = GameJudge.judge(hand)
        Assertions.assertThat(actual).isEqualTo(expected)
    }
    ...
    fun `카드의 합이 21보다 작다면 스테이 상태를 반환한다 `() {
        val expected = GameStatus.STAY
        val hand = Hand().make(HEART_TWO, HEART_QUEEN)
        val actual = GameJudge.judge(hand)
        Assertions.assertThat(actual).isEqualTo(expected)
    }
    ...
    fun `카드의 합이 21보다 크다면 버스트 상태를 반환한다 `() {
        ...
        val hand = Hand().make(HEART_TWO, HEART_QUEEN, CLOVER_NINE, HEART_ACE)
        val actual = GameJudge.judge(hand)
        Assertions.assertThat(actual).isEqualTo(expected)
    }
}
